...
CREATE INDEX idx_task_execution_logs_reference ON task_execution_logs(reference_type, reference_id) WHERE reference_type IS NOT NULL;
...
CREATE INDEX idx_bookings_reminder_sent_at ON bookings(reminder_sent_at) WHERE reminder_sent_at IS NULL AND status = 'confirmed';
CREATE INDEX idx_bookings_thank_you_sent_at ON bookings(thank_you_sent_at) WHERE thank_you_sent_at IS NULL AND status = 'completed';

-- =============================================================================
-- 3. meeting_menus テーブルへのカラム追加
-- =============================================================================
ALTER TABLE meeting_menus
    ADD COLUMN send_thank_you_email BOOLEAN DEFAULT false;
